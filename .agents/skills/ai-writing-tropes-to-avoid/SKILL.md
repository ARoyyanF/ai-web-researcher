...
description: >
  A reference guide to common AI writing patterns and tropes that make content
  sound robotic, generic, or obviously AI-generated. Use this skill whenever
  writing or reviewing prose — blog posts, documentation, articles, marketing
  copy, READMEs, emails, reports, or any text meant for human readers. Load it
  as a quality checklist before finalizing written output. This applies to ALL
  writing tasks, not just creative ones. Even technical writing and code comments
  benefit from avoiding these patterns. If the user asks you to write, rewrite,
  edit, review, or improve any text, load this skill.
---
...
**Any single trope used once might be fine. The problem is when multiple tropes appear together or when a single trope is used repeatedly. Write like a human: varied, imperfect, specific.**
...
1. **Negative parallelism** — if you find "not X, but Y" more than once, rewrite
2. **Em dash count** — if more than 3-4 in a single piece, reduce
3. **"Quietly" and magic adverbs** — cut or replace with specific language
4. **Bold-first bullets** — use plain bullets or vary the format
5. **Rhetorical questions** — if you're asking a question just to answer it, state the point directly
6. **Tricolon density** — one is fine, three in a row is a pattern
7. **Grandiose language** — would a human actually say this in conversation?
8. **Filler transitions** — "It's worth noting", "Interestingly" — just make the point
9. **Repeated metaphors** — use a metaphor once, then let it go
10. **Signposted conclusions** — let the ending speak for itself
